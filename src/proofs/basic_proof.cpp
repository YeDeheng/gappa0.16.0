...
UNARY_INTERVAL(log_updater)    { r = log(h);    }
UNARY_INTERVAL(exp_updater)    { r = exp(h);    }
UNARY_INTERVAL(log2_updater)   { r = log2(h);   }
UNARY_INTERVAL(exp2_updater)   { r = exp2(h);   }
...
#if 0
    case UOP_ABS:
      return create_theorem(1, hyps, property(real, abs(i)), std::string("abs_") += ('o' + sign(i)), &abs_updater);
#endif
    case UOP_LOG:
      if (lower(i) <= 0) return NULL;
      return create_theorem(1, hyps, property(real, log(i)), "log", &log_updater);
    case UOP_EXP:
      return create_theorem(1, hyps, property(real, exp(i)), "exp", &exp_updater);
    case UOP_LOG2:
      if (lower(i) <= 0) return NULL;
      return create_theorem(1, hyps, property(real, log2(i)), "log", &log2_updater);
    case UOP_EXP2:
      return create_theorem(1, hyps, property(real, exp2(i)), "exp", &exp2_updater);
    default:
      assert(false);
    }
    break; }
...
proof_scheme *computation_abs_scheme::factory(predicated_real const &real) {
  ...
  if (!p || p->fun || p->type == UOP_SQRT || p->type == UOP_EXP || p->type == UOP_LOG || 
      p->type == UOP_LOG2 || p->type == UOP_EXP2) return NULL;
